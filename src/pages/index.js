import Head from "next/head";
import Link from "next/link";
import styles from "../frontend/styles/Home.module.css";

export default function Home() {
  return (
    <div className={styles.container}>
      <Head>
        <title>Auth test</title>
        <meta name="description" content="Auth test :)" />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <main className={styles.main}>
        <h1 className={styles.title}>Auth Test Project</h1>

        <p className={styles.description}>
          A simple project for test authentication on Next.js, with nookies!
        </p>
        <p>
          Try accessing <code className={styles.codetwo}>/dashboard</code>{" "}
          without logging in and see what happens!
        </p>

        <div className={styles.links}>
          <Link href="/auth/login">
            <a className={styles.linkone}>Login</a>
          </Link>

          <Link href="/auth/signin">
            <a className={styles.linktwo}>Signin</a>
          </Link>
        </div>
      </main>

      {/* <footer className={styles.footer}>
        <a
          href="https://vercel.com?utm_source=create-next-app&utm_medium=default-template&utm_campaign=create-next-app"
          target="_blank"
          rel="noopener noreferrer"
        >
          Powered by{" "}
          <span className={styles.logo}>
            <Image src="/vercel.svg" alt="Vercel Logo" width={72} height={16} />
          </span>
        </a>
      </footer> */}
    </div>
  );
}
